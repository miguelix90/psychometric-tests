<x-app-layout>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.tasks.items.spatial.update', [$task, $item]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Formato recomendado: VIS_NNN (Ej: VIS_001, VIS_015)</p>
                            @error('code')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="question_text" id="question_text" rows="3" required
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('question_text') border-red-500 @enderror"
                                      placeholder="Escribe aquí la instrucción o pregunta para el participante">{{ old('question_text', $item->content['question_text'] ?? '') }}</textarea>
                            <p class="mt-1 text-sm text-gray-500">Instrucción que verá el participante (máximo 500 caracteres)</p>
                            @error('question_text')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4 p-4 border border-gray-300 rounded">
                            <label for="stimulus_image" class="block text-sm font-medium text-gray-700">Imagen de Estímulo</label>

                            @if(isset($item->content['stimulus_image']))
                                <div class="mt-2 mb-3">
                                    <p class="text-xs text-gray-500 mb-1">Imagen actual:</p>
                                    <img src="{{ asset('storage/' . $item->content['stimulus_image']) }}"
                                         alt="Estímulo actual"
                                         class="max-w-xs rounded border">
                                </div>
                            @endif

                            <input type="file" name="stimulus_image" id="stimulus_image" accept="image/*"
                                   class="mt-1 block w-full text-sm text-gray-500
                                          ...
                                          @error('stimulus_image') border-red-500 @enderror">
                            <p class="mt-1 text-sm text-gray-500">Sube una nueva imagen para reemplazar la actual (SVG, PNG, JPG). Máx: 2MB</p>
                            @error('stimulus_image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror

                            <!-- Preview -->
                            <div id="stimulus_preview" class="mt-2 hidden">
                                <p class="text-xs text-gray-500 mb-1">Vista previa nueva:</p>
                                <img src="" alt="Preview estímulo" class="max-w-xs rounded border">
                            </div>
                        </div>
                        ...
                        <div class="mb-4 p-4 border border-gray-300 rounded">
                            ...
                            <p class="text-sm text-gray-500 mb-3">Gestiona las opciones de respuesta (máximo 6 opciones)</p>

                            @for($i = 1; $i <= 6; $i++)
                                <div class="mb-3 p-3 bg-gray-50 rounded">
                                    <label for="option_{{ $i }}" class="block text-sm font-medium text-gray-700">Opción {{ $i }}</label>

                                    @if(isset($item->content['options'][(string)$i]))
                                        <div class="mt-2 mb-2">
                                            <p class="text-xs text-gray-500 mb-1">Imagen actual:</p>
                                            <img src="{{ asset('storage/' . $item->content['options'][(string)$i]) }}"
                                                 alt="Opción {{ $i }}"
                                                 class="max-w-xs h-24 object-contain rounded border">
                                        </div>
                                    @endif
                                    ...
                                    @if(isset($item->content['options'][(string)$i]))
                                        <div class="mt-2">
                                            <label class="flex items-center">
                                                <input type="checkbox" name="delete_option_{{ $i }}" value="1"
                                                       class="rounded border-gray-300">
                                                <span class="ml-2 text-sm text-gray-600">Eliminar opción {{ $i }}</span>
                                            </label>
                                        </div>
                                    @endif
                        ...
                        <div class="mb-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Indica el número de la opción correcta (1-6)</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Preview de imagen de estímulo
            const stimulusInput = document.getElementById('stimulus_image');
            const stimulusPreview = document.getElementById('stimulus_preview');

            if (stimulusInput && stimulusPreview) {
                stimulusInput.addEventListener('change', function(e) {
                    const file = e.target.files[0];
                    if (file) {
                        const reader = new FileReader();
                        reader.onload = function(e) {
                            stimulusPreview.querySelector('img').src = e.target.result;
                            stimulusPreview.classList.remove('hidden');
                        }
            ...
            // Preview de opciones (números 1-6)
            for (let i = 1; i <= 6; i++) {
