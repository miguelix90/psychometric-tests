        ...
        <div class="bg-white rounded-lg shadow-xl p-8 lg:p-12">
            <div class="text-center mb-6">
                ...
                <p class="text-gray-600">
                    Encuentra la pieza que completa la matriz
                </p>
            </div>
            <div class="flex justify-center">
                <img src="{{ Storage::url($item->content['matrix_image']) }}"
                     alt="Matriz"
...
<div class="bg-white rounded-lg shadow-lg p-8">
    ...
    <div class="grid {{ $gridClass }} gap-4 md:gap-6 mb-8 max-w-6xl mx-auto">
        @foreach($item->content['options'] as $optionNumber => $optionPath)
            @if(!empty($optionPath))
...
<script>
    ...
    const config = {
        ...
        @elseif(isset($demoMode) && $demoMode)
            ...
            itemId: {{ $item->id }},
            csrfToken: '{{ csrf_token() }}',
            submitUrl: '{{ route('admin.demo.matrix.response', ['itemId' => $item->id]) }}',
            demoMode: true,
            demoType: 'task'
        @else
            // Modo Normal
            ...
            itemId: {{ $item->id }},
            csrfToken: '{{ csrf_token() }}',
            submitUrl: '{{ route('test.matrix.item.submit', [$testSessionTask, $item]) }}',
            demoMode: false,
            demoType: null
        @endif
    };
